	...
	T: Deserialize<'a>,
{
	...
	let (trailing, _) = parse::parse_optional_edn(s, s.len() - remaining.len())?;
	if trailing.is_some() {
...
impl<'de, 'edn: 'de> de::Deserializer<'de> for Edn<'edn> {
		...
		V: Visitor<'de>,
	{
		match &mut self {
			...
			| Edn::Str(Cow::Owned(value)) => visitor.visit_string(mem::take(value)),
			...
			Edn::Vector(list) | Edn::List(list) => {
				list.reverse();
				Ok(visitor.visit_seq(SeqEdn::new(mem::take(list)))?)
			}
			Edn::Map(map) => visitor.visit_map(MapEdn::new(mem::take(map))),
			Edn::Set(set) => {
				let mut s: Vec<Edn<'_>> = mem::take(set).into_iter().collect();
				s.reverse();
		...
		V: Visitor<'de>,
	{
		match self {
			Edn::Nil => visitor.visit_unit(),
			Edn::Map(ref map) if map.is_empty() => visitor.visit_unit(),
	...
	where
		V: Visitor<'de>,
	{
		let Edn::Tagged(tag, edn) = &mut self else {
		...
		visitor.visit_enum(EnumEdn::new(mem::replace(edn.as_mut(), Edn::Nil), variant))
	}
...
impl<'de, 'edn: 'de> EnumAccess<'de> for EnumEdn<'edn> {
		...
		V: DeserializeSeed<'de>,
	{
		let val = match self.variant {
			Cow::Borrowed(value) => {
				seed.deserialize(de::value::BorrowedStrDeserializer::<Error>::new(value))?
			}
			Cow::Owned(value) => seed.deserialize(value.into_deserializer())?,
		};
