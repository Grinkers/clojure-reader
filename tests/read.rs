...
fn into_owned_handles_deep_nesting_without_recursion() {
	...
	fn unwrap(nesting: Nesting, mut value: Edn<'static>) -> Edn<'static> {
		match (nesting, &mut value) {
			(Nesting::Vector, Edn::Vector(values)) | (Nesting::List, Edn::List(values)) => {
				assert_eq!(values.len(), 1);
				values.pop().unwrap()
			}
			(Nesting::Set, Edn::Set(values)) => {
				assert_eq!(values.len(), 1);
				values.pop_first().unwrap()
			}
			(Nesting::Map, Edn::Map(values)) => {
				assert_eq!(values.len(), 1);
			...
			(Nesting::Tagged, Edn::Tagged(Cow::Owned(tag), value)) => {
				...
				core::mem::replace(value.as_mut(), Edn::Nil)
			}
			_ => panic!(),
		}
	}
